...
import { AiOutlineEdit, AiOutlineMail, AiFillCloseCircle, AiOutlineRightCircle } from 'react-icons/ai';
import { BiSearch, BiUserCircle } from 'react-icons/bi';
import { Link } from '@inertiajs/react';
import { BsArrowRightCircle, BsArrowUpCircle, BsTelephone } from 'react-icons/bs';
...
const ClientProfile = () => {
    ...
    const [openDeleteGeneralNoteModal, setOpenDeleteGeneralNoteModal] = useState(false);
    const [openCreateSubClientModal, setOpenCreateSubClientModal] = useState(false);
        ...
        <div>
            ...
            <div className="flex my-4 gap-6">
                <div className="flex flex-4 flex-col pt-8 pb-14 px-10 justify-center bg-white items-center rounded-lg">
                    <BiUserCircle size={20} className="flex justify-center items-center" />
                ...
                <div className="flex-1 flex-col py-8 bg-white rounded-lg">
                    <div className="flex justify-between px-6">
                        ...
                        <button onClick={() => setOpenNewPOCModal(true)} className="text-xl font-bold text-[#00B4AD]">+ Add New</button>
                    </div>
                            ...
                            POCData.map((poc, i) => (
                                <div key={i}>
                                    <div className="flex justify-between pt-4">
                                        <span className="text-md font-bold">{poc.name || 'POC 1'}</span>
                                    ...
                                    <span className="text-sm text-[#00B4AD] font-bold py-2">{poc.personName || 'Albert'}</span>
                ...
                <div className="flex-1 flex-col py-8 bg-white rounded-lg">
                    <div className="flex justify-between px-6">
                        ...
                        <button onClick={() => setOpenNewGeneralNotesModal(true)} className="text-xl font-bold text-[#00B4AD]">+ Add New</button>
                    </div>
                    <div className='flex flex-col max-h-[30vh] overflow-y-auto mt-4 gap-4 px-6'>
                        {
                            GeneralNotes.map((note, i) => (
                                <div key={i} className='flex flex-col gap-2'>
                                    ...
                                    <div className='text-[#303030] text-[12px]'>Updated on {note.dateTime} by <span className='text-primary font-bold'>{note.updatedBy}</span></div>
                                </div>
                            ))
                        }
                    </div>
                    ...
                    <div className="bg-white p-4 rounded-lg mt-3">
                        <div className="flex justify-between">
                            <div className="flex flex-col">
                                <span className="text-[#00B4AD] text-sm font-bold">Contract ID 240920-CO-00021</span>
                            ...
                            <div className="flex justify-between">
                                ...
                                <div className="flex justify-end flex-col text-xs font-bold">
                                    ...
                                    <div className="flex gap-2 pt-2">
                                        <img src={SampleProfileImg} alt="Profile" className="rounded-full" />
                    ...
                    <table className="w-full text-black relative">
                        <thead className="relative bg-[#F0F0F0] rounded-full">
                            <tr>
                                ...
                                <th className="px-4 py-2 sticky left-0 min-w-[150px] text-[#455361] text-left">Contact Person</th>
                                <th className="px-4 py-2 sticky left-0 min-w-[150px] text-[#455361] text-left">Contact</th>
                                <th className="px-4 py-2 sticky left-0 min-w-[150px] text-[#455361] text-left">Address</th>
                                <th className="px-4 py-2 sticky left-0 min-w-[150px] text-[#455361] text-left">Sub-Clients</th>
                                <th className="px-4 py-2 sticky left-0 min-w-[150px] text-[#455361] text-left">Billing Address</th>
                        ...
                        <tbody className="relative">
                            {
                                clientsData.map((item, i) => (
                                    <tr className={`text-[#455361] ${i % 2 === 1 && 'bg-white rounded-full'}`} key={i}>
                                        ...
                                        <td className='px-4 py-4 my-1 max-w-[200px]'>
                                            <div className='flex flex-col justify-center'>
                                                <span className='text-[15px]'>{item.contactPerson.name}</span>
                                                <span className={`${item.contactPerson.type === 'Residential' ? 'bg-primary' : 'bg-secondary'} text-white text-[13px] mt-2 rounded-full text-center px-2 w-max`}>{item.contactPerson.type}</span>
                                            </div>
                                        </td>
                                        <td className='px-4 py-4 my-1 max-w-[200px]'>
                                            <div className='flex flex-col justify-center'>
                                        ...
                                        <td className={`z-[${i + 1}] px-4 py-4 my-1 max-w-[200px]`}>
                                            <div className='flex text-primary flex-row items-center relative'>
                                                <span className='text-primary text-[14px]'>{item.subClients.length}</span>
                                                <button className={`z-${i + 1} ml-2`} onClick={() => handleSubClients(i)}>
                                                    <BiInfoCircle size={20} />
                                                </button>
                                                <div className={`${i === showSubClients ? 'flex flex-col' : 'hidden'} absolute w-max text-white bg-[#838383] max-h-[200px] top-5 left-4 rounded-sm p-2 z-[${i + 400}]`}>
                                                    <button onClick={() => setShowSubClients(null)} className='self-end'>
                                                        <AiFillCloseCircle className='text-white' />
                                                    </button>
                                                    <span>Sub Client:</span>
                                                    {
                                                        item.subClients.map(subClient => <span className='p-1' key={subClient.id}>{subClient.name}</span>)
                                                    }
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td className='px-4 py-4 my-1'>
                                            <Link href={'/client-details'} method="get" as="button" type="button" className='flex flex-col justify-center'>
                                                <AiOutlineRightCircle size={20} className='text-secondary' />
                                            </Link>
                                        </td>
                                    </tr>
                                ))
