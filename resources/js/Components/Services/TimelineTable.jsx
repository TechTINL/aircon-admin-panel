  ...
  const [{ isDragging }, drag] = useDrag({
    ...
    item: { task },
      ...
      if (monitor.didDrop()) {
        ...
        alert(`You moved ${item.task.name} to ${dropResult.hour}`);
      }
    },
    collect: (monitor) => ({
  ...
  return (
    <div
      ...
      style={{
        ...
        borderLeft: `8px solid ${getBorderColor(task.status)}`,
      }}
      className="cells bg-[#F8F8F8] py-3 rounded tasks mx-2 w-48"
    >
      <div className="flex justify-between">
        <p className="mx-2 text-sm font-bold">{task.name}</p>
        <p className="mx-2 text-sm text-yellow">{task.status}</p>
      </div>
      <p className="max-w-full px-2 text-sm overflow-hidden whitespace-nowrap">{task.address}</p>
...
function TimelineTable() {
  ...
  const moveTask = (task, employee, hour) => {
    setTasks((prevTasks) =>
      prevTasks.map((t) =>
        t.id === task.id ? { ...t, assign: { employeeId: employee.id, hour } } : t
      )
    );
  };
          ...
          <tr key={employee.id}>
            ...
            {hours.map((hour) => {
              let task = filteredTasks.find(
                (t) =>
                  t.assign &&
                  t.assign.employeeId === employee.id &&
                  t.assign.hour === hour
              );
