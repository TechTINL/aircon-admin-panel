...
export const TimelineTable = ({
    ...
    const renderColumns = (isHeader = false) => {
        const columns = [];
        for (let i = 8; i <= 20; i++) {
            columns.push(<th className="py-2 min-w-[100px] border border-[#BCBDC0]">
                {isHeader && <span className="text-[12px] text-black">{formatTime(`${i}:00`)}</span>}
            </th>);
            if (i < 20) {
                columns.push(<th className="py-2 min-w-[100px] max-w-[100px] border border-[#BCBDC0]">
                    {isHeader && <span className="text-[12px] text-black">{formatTime(`${i}:30`)}</span>}
                </th>);
            }
        }
        return columns;
    ...
    const getTaskClassNames = (task, index) => {
        ...
        startHour = Number(startHour) + (task?.start?.split(' ')[1] === 'PM' ? 12 : 0);
    ...
    const renderTaskCard = (task, i) => {
        ...
        return (
            ...
            >
                <div className={`rounded-lg h-full w-2`} style={{ backgroundColor: styles.tileBgColor }}></div>
                <div className='flex flex-col p-2'>
                    <span className={`font-bold`} style={{ color: styles.titleTextColor }}>
                        {task?.clientName}
                        <span className='italic text-[12px] font-thin pl-2' style={{ color: styles.statusTextColor }}>{task.status}</span>
                    </span>
                    <span className={``} style={{ color: styles.textColor }}>{task?.addresss}</span>
                </div>
            </td>
        );
    }
        ...
        <div className="w-full h-[80vh] overflow-x-auto overflow-y-auto rounded-xl">
            ...
            <table className="min-w-full bg-table-header-bg text-black relative">
                <thead className="relative">
                    <tr>
                        <th className="px-4 py-2 sticky left-0 min-w-[150px] bg-table-header-bg" rowSpan={2}>Time</th>
                        <tr>
                            <th className="px-4 py-2 sticky top-0 h-10 overflow-hidden border border-[#BCBDC0] border-t-0" colSpan={25}>
                            </th>
                        </tr>
                        <tr className="h-10">
                            {...renderColumns(true)}
                ...
                <tbody className="relative bg-white">
                    {
                        tasks.map((task, i) => <tr>
                            <td className="px-4 py-2 sticky left-0 bg-[#F8F8F8] border border-[#F2F2F2] z-40">{task.name}</td>
