  ...
  return (
    <Dropdown>
      <Dropdown.Trigger>
        ...
        >
          {startIcon}
          <span className="px-2 font-bold text-[14px]">{selectedItem?.label || label || 'Select Item'}</span>
      ...
      <Dropdown.Content>
        <div className='flex flex-col w-full max-h-[200px] overflow-y-scroll z-[500]'>
          <div>
            {
              items.map((item, i) => (
                <button key={i} onClick={() => onItemSelect(item)} className={`flex text-black items-start p-2 ${selectedItem?.value === item.value && 'text-secondary'}`}>
                  {item.label}
                </button>
              ))
            }
