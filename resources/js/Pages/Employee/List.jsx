...
const List = ({ auth }) => {
    ...
    const handleCheckJobPositionFilters = (item, checked) => {
        if (checked) {
            ...
            setCheckedJobPositionFilters(checkedJobPositionFilters.filter(filter => filter !== item.value));
        }
    }
                                ...
                                <tbody className="relative">
                                    {
                                        DATA.map((item, i) => (
                                            <tr className={`text-[#455361] text-[14px] ${i % 2 === 1 && 'bg-white rounded-full'}`} style={{
                                                ...
                                                <td className='px-4 py-2 w-[180px]'>{JOB_POSTION[item.jobPosition]}</td>
                                                <td className='px-4 py-2 max-w-[200px]'>{item.team}</td>
                                                <td className='px-4 py-2 max-w-[200px]'>{item.organisation || '-'}</td>
